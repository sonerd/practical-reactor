...
public class c8_Sinks extends SinksBase {
    ...
    public void single_shooter() {
        final Sinks.One<Boolean> sink = Sinks.one();
    ...
    public void single_subscriber() {
        final Sinks.Many<Integer> sinks = Sinks.many().unicast().onBackpressureBuffer();
        Flux<Integer> measurements = sinks.asFlux();
        submitOperation(() -> {
            List<Integer> measures_readings = get_measures_readings(); //don't change this line
            measures_readings.forEach(i -> sinks.tryEmitNext(i));
    ...
    public void it_gets_crowded() {
        ...
        submitOperation(() -> {
            List<Integer> measures_readings = get_measures_readings(); //don't change this line
            measures_readings.forEach(i -> sink.tryEmitNext(i));
