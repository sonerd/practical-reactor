...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void unit_of_work() {
        Flux<Task> taskFlux = taskQueue().flatMap(
                t -> t.execute()
                      .then(t.commit())
    ...
    public void billion_dollar_mistake() {
        Flux<String> content = getFilesContent()
                ...
                .onErrorContinue((e, f) -> System.out.println("Error occurred while reading file: " + e.getMessage()));
    ...
    public void resilience() {
        Flux<String> content = getFilesContent().flatMap(f -> f.onErrorResume(e -> Mono.empty()));
