...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((first, mailbox) -> {
    ...
    public void prettify() {
        Mono<Boolean> successful = openFile().then(writeToFile("bla"))
                                             .then(closeFile())
                                             .thenReturn(true);
    ...
    public void acid_durability() {
        Flux<String> committedTasksIds = tasksToExecute().concatMap(t -> t.flatMap(tId -> commitTask(tId).thenReturn(tId)));
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(),
                                             s -> s,
                                             e -> StreamingConnection.closeConnection());
